...
public class MyDataBase {
    ...
    public ArrayList<Cuns> getArray(){
        ...
        Cursor cursor=myDatabase.rawQuery("select ids,title, content,times from mybook" , null);
        cursor.moveToFirst();
        while(!cursor.isAfterLast()){
            ...
            Cuns cun = new Cuns(id, title, content, times);
            array.add(cun);
            cursor.moveToNext();
        }
        myDatabase.close();
    ...
    public Cuns getTiandCon(int id){
        myDatabase = myHelper.getWritableDatabase();
        Cursor cursor=myDatabase.rawQuery("select title,content,times from mybook where ids='"+id+"'" , null);
        cursor.moveToFirst();
        String title=cursor.getString(cursor.getColumnIndex("title"));
        String content=cursor.getString(cursor.getColumnIndex("content"));
        String times = cursor.getString(cursor.getColumnIndex("times"));
        Cuns cun=new Cuns(title,content,times);
        myDatabase.close();
        return cun;
    }
